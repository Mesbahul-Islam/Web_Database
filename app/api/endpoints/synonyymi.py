...
from app.models.synonyymi import Synonyymi as Model  # synonyymi: synonym
from app.schemas.synonyymi import Synonyymi as Schema, SynonyymiCreate as SchemaCreate  # synonyymi: synonym
...
async def update_one(synonyymin_nro: int, payload: SchemaCreate, db: Session = Depends(get_db)):
    return await update_item(payload, db, Model, "synonyymin_nro", synonyymin_nro)

@router.delete("/{synonyymin_nro}", status_code=204)
async def delete_one(synonyymin_nro: int, db: Session = Depends(get_db)):
