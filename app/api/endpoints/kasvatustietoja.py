...
async def update_one(lisatietojen_nro_kasvatus: int, payload: SchemaCreate, db: Session = Depends(get_db)):
...
async def delete_one(lisatietojen_nro_kasvatus: int, db: Session = Depends(get_db)):
