...
from app.models.suomalainen_kasvupaikka import SuomalainenKasvupaikka as Model
from app.schemas.suomalainen_kasvupaikka import SuomalainenKasvupaikka as Schema, SuomalainenKasvupaikkaPage as SchemaPage, SuomalainenKasvupaikkaCreate as SchemaCreate

router = APIRouter()

@router.get("/", response_model=SchemaPage)
@cached_list("suomalainen_kasvupaikka")
def read_all(
    request: Request,
...
async def create_one(payload: SchemaCreate, current_user: Annotated[User, Depends(get_current_user)], db: Session = Depends(get_db)):
    invalidate_endpoint_cache("suomalainen_kasvupaikka")
...
async def update_one(id: int, payload: SchemaCreate, current_user: Annotated[User, Depends(get_current_user)], db: Session = Depends(get_db)):
    invalidate_endpoint_cache("suomalainen_kasvupaikka")
    return await update_item(payload, db, Model, "suomalaisen_kasvupaikan_nro", id)

@router.delete("/{id}", status_code=204)
async def delete_one(id: int, current_user: Annotated[User, Depends(get_current_user)], db: Session = Depends(get_db)):
    invalidate_endpoint_cache("suomalainen_kasvupaikka")
    await delete_item(db, Model, "suomalaisen_kasvupaikan_nro", id)