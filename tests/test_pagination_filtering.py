...
class TestListEndpointShape:
    ...
    def test_all_list_endpoints_return_200(self, client):
        for route in client.app.router.routes:
            ...
            if route.path in {
                ...
                "/docs/oauth2-redirect", "/api/cache-stats",
    ...
    def test_taksoni_list_returns_page_envelope(self, client):
        body = client.get("/api/taksoni/", params={"page": 1, "page_size": 5}).json()
        assert _is_page_envelope(body)
        assert len(body["items"]) == 5

    def test_heimo_list_returns_page_envelope(self, client):
        body = client.get("/api/heimo/", params={"page": 1, "page_size": 3}).json()
        assert _is_page_envelope(body)

    def test_hankintatiedot_list_returns_page_envelope(self, client):
        body = client.get("/api/hankintatiedot/", params={"page": 1, "page_size": 5}).json()
        assert _is_page_envelope(body)
...
class TestPagination:
    ...
    def test_default_page_is_1(self, client):
        body = client.get("/api/taksoni/", params={"page_size": 5}).json()
        assert body["page"] == 1

    def test_page_size_controls_item_count(self, client):
        body5 = client.get("/api/taksoni/", params={"page": 1, "page_size": 5}).json()
        body10 = client.get("/api/taksoni/", params={"page": 1, "page_size": 10}).json()
    ...
    def test_page_2_has_different_items_from_page_1(self, client):
        ids1 = {i["taksonin_nro"] for i in
                client.get("/api/taksoni/", params={"page": 1, "page_size": 5}).json()["items"]}
        ids2 = {i["taksonin_nro"] for i in
                client.get("/api/taksoni/", params={"page": 2, "page_size": 5}).json()["items"]}
    ...
    def test_pages_field_equals_ceil_total_over_page_size(self, client):
        body = client.get("/api/taksoni/", params={"page": 1, "page_size": 7}).json()
    ...
    def test_total_is_same_on_every_page(self, client):
        t1 = client.get("/api/taksoni/", params={"page": 1, "page_size": 5}).json()["total"]
        t2 = client.get("/api/taksoni/", params={"page": 2, "page_size": 5}).json()["total"]
        t3 = client.get("/api/taksoni/", params={"page": 3, "page_size": 5}).json()["total"]
        assert t1 == t2 == t3

    def test_page_beyond_last_returns_empty_items(self, client):
        total = client.get("/api/taksoni/", params={"page": 1, "page_size": 1}).json()["total"]
        body = client.get("/api/taksoni/", params={"page": total + 100, "page_size": 1}).json()
        assert body["items"] == []
        assert body["total"] == total  # total unchanged

    def test_page_size_1_returns_exactly_one_item(self, client):
        body = client.get("/api/taksoni/", params={"page": 1, "page_size": 1}).json()
    ...
    def test_last_page_may_have_fewer_items_than_page_size(self, client):
        ...
        body = client.get("/api/taksoni/", params={"page": last_page, "page_size": page_size}).json()
    ...
    def test_page_less_than_1_returns_422(self, client):
        assert client.get("/api/taksoni/", params={"page": 0}).status_code == 422

    def test_page_size_less_than_1_returns_422(self, client):
        assert client.get("/api/taksoni/", params={"page_size": 0}).status_code == 422

    def test_response_echoes_back_requested_page_and_page_size(self, client):
        body = client.get("/api/taksoni/", params={"page": 3, "page_size": 7}).json()
        assert body["page"] == 3
    ...
    def test_different_page_sizes_cover_same_total(self, client):
        ...
        for p in range(1, pages + 1):
            items = client.get("/api/taksoni/", params={"page": p, "page_size": page_size}).json()["items"]
            collected += len(items)
...
class TestColumnFiltering:
    ...
    def test_filter_narrows_results_vs_unfiltered(self, client, db_session):
        ...
        unfiltered = client.get(
            "/api/alkuperaa_koskevat_tiedot/", params={"page": 1, "page_size": 1}
        ).json()["total"]
        filtered = client.get(
            "/api/alkuperaa_koskevat_tiedot/",
    ...
    def test_unknown_column_filter_is_silently_ignored(self, client):
        r = client.get("/api/taksoni/", params={"page": 1, "page_size": 5, "does_not_exist": "abc"})
        assert r.status_code == 200
        assert _is_page_envelope(r.json())

    def test_filter_with_no_matching_rows_returns_zero_total(self, client):
        body = client.get(
            "/api/taksoni/", params={"page": 1, "page_size": 10, "taksonin_nro": -99999}
        ).json()
        assert body["total"] == 0
        assert body["items"] == []
...
class TestSearch:
    ...
    def test_search_returns_results(self, client, db_session):
        ...
        body = client.get("/api/taksoni/", params={"page": 1, "page_size": 50, "search": term}).json()
        assert body["total"] > 0
    ...
    def test_search_is_case_insensitive(self, client, db_session):
        ...
        term = row.tieteellinen_nimi[:5]
        lower = client.get("/api/taksoni/", params={"page": 1, "page_size": 1, "search": term.lower()}).json()["total"]
        upper = client.get("/api/taksoni/", params={"page": 1, "page_size": 1, "search": term.upper()}).json()["total"]
        assert lower == upper
    ...
    def test_search_reduces_result_count(self, client, db_session):
        ...
        total_all = client.get("/api/taksoni/", params={"page": 1, "page_size": 1}).json()["total"]
        total_search = client.get(
            "/api/taksoni/", params={"page": 1, "page_size": 1, "search": row.tieteellinen_nimi[:5]}
        ).json()["total"]
        assert total_search <= total_all

    def test_empty_search_returns_same_as_no_search(self, client):
        no_search = client.get("/api/taksoni/", params={"page": 1, "page_size": 1}).json()["total"]
        empty_search = client.get(
            "/api/taksoni/", params={"page": 1, "page_size": 1, "search": ""}
        ).json()["total"]
    ...
    def test_nonsense_search_returns_empty(self, client):
        body = client.get(
            "/api/taksoni/", params={"page": 1, "page_size": 10, "search": "xyzzy_not_a_real_name_12345"}
        ).json()
        assert body["total"] == 0
        assert body["items"] == []
...
class TestNestedFKRoutes:
    ...
    def test_nested_route_returns_page_envelope(self, client, db_session):
        ...
        r = client.get(f"/api/hankintatiedot/{hankinta.hankintaID}/alkuperaa_koskevat_tiedot")
        assert r.status_code == 200
        assert _is_page_envelope(r.json())
    ...
    def test_nested_route_filters_by_parent_pk(self, client, db_session):
        ...
        body = client.get(
            f"/api/hankintatiedot/{hankinta.hankintaID}/alkuperaa_koskevat_tiedot",
    ...
    def test_nested_route_taksoni_synonyymi(self, client, db_session):
        ...
        body = client.get(
            f"/api/taksoni/{taksoni.taksonin_nro}/synonyymi",
            params={"page": 1, "page_size": 200},
        ).json()
    ...
    def test_nested_route_heimo_taksoni(self, client, db_session):
        ...
        body = client.get(
            f"/api/heimo/{heimo.jarjestysnumero}/taksoni",
    ...
    def test_nested_route_sijoituspaikka_tarkastusmerkinta(self, client, db_session):
        ...
        body = client.get(
            f"/api/sijoituspaikka/{sijoitus.sijoituspaikan_nro}/tarkastusmerkinta",
    ...
    def test_nested_route_nonexistent_parent_returns_empty(self, client):
        body = client.get(
            "/api/hankintatiedot/-99999/alkuperaa_koskevat_tiedot",
            params={"page": 1, "page_size": 10},
        ).json()
        assert body["total"] == 0
        assert body["items"] == []
...
class TestOpenAPIFilterParams:
    ...
    def _get_taksoni_params(self, client) -> set[str]:
        ...
        return {p["name"] for p in schema["paths"]["/api/taksoni/"]["get"].get("parameters", [])}
    ...
    def test_alkuperaa_koskevat_tiedot_exposes_hankintaid_filter(self, client):
        ...
        params = {
            p["name"] for p in
            schema["paths"]["/api/alkuperaa_koskevat_tiedot/"]["get"].get("parameters", [])
        }
    ...
    def test_filter_params_have_correct_types_in_schema(self, client):
        ...
        params = {
            p["name"]: p
            for p in schema["paths"]["/api/taksoni/"]["get"].get("parameters", [])
        }
